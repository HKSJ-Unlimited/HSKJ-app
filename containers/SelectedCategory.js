...
const SelectedCategory = ({navigation}) => {
  ...
  const _matchThumbsToCategory = (response, res) => {
    ...
    for (let i = 0; i < thumbs.length; i++) {
      ...
      if (obj !== undefined) {
        const regex = /=s220/gi;
        const newThumbLink = (obj.size = thumbs[i].thumbnailLink.replace(
          regex,
          '=s720',
        ));
        ...
        files[i].size =
          'https://image.shutterstock.com/image-photo/grunge-black-background-texture-space-260nw-373662322.jpg';
      }
    }
    setRes(files);
  };
